...
<head>
    ...
    <title>{{ __('Fasilitas Pelatihan') }} - PT GeoINHance</title>
    ...
    <main class="flex-grow">
        <section class="relative bg-geo-dark text-white py-20 lg:py-24 overflow-hidden">
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="max-w-3xl">
                    <span class="text-blue-400 font-bold tracking-wider uppercase text-sm mb-3 block">{{ __('Program Pelatihan Geo-Engineering') }}</span>
                    <h1 class="text-4xl md:text-5xl font-extrabold leading-tight mb-6">
                        {{ __('Fasilitas Lengkap untuk') }} <span class="text-blue-400">{{ __('Pengalaman Belajar Maksimal') }}</span>
                    </h1>
                    <p class="text-lg text-gray-300 leading-relaxed mb-8">
                        {{ __('Kami tidak hanya memberikan materi teori, tetapi juga memastikan Anda mendapatkan fasilitas pendukung terbaik agar siap mengaplikasikan ilmu geoteknik dan rekayasa struktur di dunia kerja nyata.') }}
                    </p>
                    <div class="flex flex-wrap gap-4">
                        ...
                        <a href="{{ route('training.silabus') }}" class="bg-transparent border border-gray-400 hover:border-white text-white font-semibold py-3 px-6 rounded-md transition flex items-center gap-2">
                            <i class="fa-solid fa-book"></i> {{ __('Lihat Silabus') }}
                        </a>
                    </div>
                </div>
            </div>
        ...
        <section class="py-20 bg-gray-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="text-center max-w-3xl mx-auto mb-16">
                    <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ __('Apa Saja yang Anda Dapatkan?') }}</h2>
                    <p class="text-gray-600">{{ __('Peserta pelatihan akan dibekali dengan berbagai fasilitas standar industri untuk menunjang praktik pemodelan numerik dan analisis rekayasa.') }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-xl p-8 shadow-sm border border-gray-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Lisensi Software Trial') }}</h3>
                        <p class="text-gray-600 leading-relaxed mb-4 text-sm">
                            {{ __('Tidak perlu khawatir jika Anda belum memiliki *software*. Kami menyediakan lisensi *trial* resmi dan legal selama masa pelatihan berlangsung.') }}
                        </p>
                    ...
                    <div class="bg-white rounded-xl p-8 shadow-sm border border-gray-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Modul & Studi Kasus') }}</h3>
                        <p class="text-gray-600 leading-relaxed mb-4 text-sm">
                            {{ __('Dapatkan modul eksklusif berisi materi komprehensif, mulai dari dasar teori hingga *step-by-step* penyelesaian studi kasus proyek nyata.') }}
                        </p>
                        <ul class="space-y-3 text-sm text-gray-700 font-medium">
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('Modul Cetak (Hardcopy) *') }}</li>
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('E-Book / Softcopy (PDF)') }}</li>
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('Dataset Proyek untuk Praktik') }}</li>
                        </ul>
                        <p class="text-[11px] text-gray-400 mt-5 italic">{{ __('* Khusus pelatihan tatap muka (offline)') }}</p>
                    </div>
                    ...
                    <div class="bg-white rounded-xl p-8 shadow-sm border border-gray-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Sertifikat Kelulusan') }}</h3>
                        <p class="text-gray-600 leading-relaxed mb-4 text-sm">
                            {{ __('Tingkatkan kredibilitas profesional Anda. Setiap peserta yang menyelesaikan pelatihan dan tugas akhir akan mendapatkan sertifikat resmi.') }}
                        </p>
                        <ul class="space-y-3 text-sm text-gray-700 font-medium">
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('Sertifikat Fisik Berhologram *') }}</li>
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('E-Certificate (High-Res PDF)') }}</li>
                            <li class="flex items-center gap-3"><i class="fa-solid fa-check text-green-500"></i> {{ __('Transkrip Jam Pembelajaran (JP)') }}</li>
                        </ul>
                    </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-blue-900 rounded-2xl p-8 md:p-12 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
                    ...
                    <div class="z-10 text-center md:text-left">
                        <h3 class="text-2xl md:text-3xl font-bold text-white mb-2">{{ __('Siap Untuk Meningkatkan Skill Anda?') }}</h3>
                        <p class="text-blue-200 text-sm md:text-base">{{ __('Bergabunglah dengan ratusan *engineer* lainnya yang telah mengikuti pelatihan kami.') }}</p>
                    </div>

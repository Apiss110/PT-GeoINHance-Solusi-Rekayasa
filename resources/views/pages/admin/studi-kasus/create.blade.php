<x-app-layout>
    <div class="container mx-auto px-6 py-8">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h3 class="text-gray-700 text-3xl font-medium">Tambah Case Study</h3>
                <p class="text-gray-500 text-sm mt-1">Unggah berkas dokumen studi kasus teknik baru</p>
            </div>
            <div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-6">
            <form action="{{ route('admin.studi-kasus.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" 
                            ...
                            placeholder="Contoh: Case Study: Cantilever Retaining Wall Failure Analysis" required>
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="sector" id="sector" 
                            ...
                            <option value="Geotechnical" {{ old('sector') == 'Geotechnical' ? 'selected' : '' }}>Geotechnical</option>
                            <option value="Structural" {{ old('sector') == 'Structural' ? 'selected' : '' }}>Structural</option>
                            <option value="Infrastructure" {{ old('sector') == 'Infrastructure' ? 'selected' : '' }}>Infrastructure</option>
                        </select>
                        @error('sector')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="year" id="year" value="{{ old('year', date('Y')) }}" min="2000" max="{{ date('Y') + 1 }}"
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">File Dokumen (PDF) <span class="text-red-500">*</span></label>
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600 justify-center">
                                    <label for="file_pdf" class="relative cursor-pointer bg-white rounded-md font-medium text-[#0e1d82] hover:text-[#0e1d82]/80 focus-within:outline-none">
                                        <span>Unggah file berkas</span>
                                        <input id="file_pdf" name="file_pdf" type="file" accept="application/pdf" class="sr-only" required onchange="updateFileName(this)">
                                    </label>
                                </div>
                                <p class="text-xs text-gray-500">Hanya menerima format PDF (Maks. 10MB)</p>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="4" 
                            ...
                            placeholder="Tulis ringkasan singkat mengenai cakupan analisis studi kasus teknik ini...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end pt-4 border-t border-gray-100 space-x-3">
                    <button type="reset" class="px-5 py-2.5 border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition">
                        Reset Form
                    </button>

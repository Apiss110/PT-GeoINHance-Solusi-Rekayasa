...
<div class="container mx-auto p-6 max-w-7xl">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <a href="{{ route('admin.project-progress.create') }}" class="px-4 py-2 bg-[#0e1d82] text-white rounded-lg hover:bg-[#0e1d82]/90 font-medium text-sm flex items-center shadow-sm transition">
                        ...
                        <tr class="hover:bg-slate-50/60 transition">
                            ...
                            <td class="p-4">
                                @if($item->status == 'completed')
                                    <span class="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 text-[11px] px-2.5 py-1 rounded-full font-bold">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Selesai
                                    </span>
                                @elseif($item->status == 'in_progress')
                                    <span class="inline-flex items-center gap-1 bg-blue-50 text-blue-700 text-[11px] px-2.5 py-1 rounded-full font-bold">
                                        <span class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span> Dalam Proses
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 bg-slate-100 text-slate-600 text-[11px] px-2.5 py-1 rounded-full font-bold">
                            ...
                            <td class="p-4">
                                <div class="flex items-center justify-center gap-1.5">
                                    {{-- Icon Detail (Checklist/Show) --}}
                                    <a href="{{ route('admin.project-progress.show', $item->id) }}" 
                                       class="text-emerald-600 hover:text-emerald-900 bg-emerald-50 p-2 rounded-lg transition"
                                    ...
                                    {{-- Icon Hapus --}}
                                    <form action="{{ route('admin.project-progress.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data progres ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 

...
<div class="container mx-auto p-6 max-w-5xl">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Tambah Progres Proyek</h1>
            <p class="text-xs text-gray-500 mt-1">Input informasi proyek beserta tahapan dan poin metodologinya secara dinamis.</p>
        </div>
    ...
    {{-- Form Utama --}}
    <form action="{{ route('admin.project-progress.store') }}" method="POST" enctype="multipart/form-data" id="projectForm">
        @csrf
        ...
        <div class="bg-white p-6 rounded-xl shadow mb-6 border border-gray-100">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ old('title') }}" class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 text-sm" placeholder="Contoh: Perencanaan Penanganan Defect Jalan Tol Ciawi-Sukabumi" required>
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Pengerjaan</label>
                <textarea name="description" rows="3" class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 text-sm" placeholder="Catatan umum atau rincian singkat proyek...">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="number" id="percentageInput" name="percentage" min="0" max="100" value="{{ old('percentage', 0) }}" class="w-full border border-gray-300 rounded-lg p-2.5 bg-gray-100 font-bold text-blue-700 text-sm cursor-not-allowed focus:outline-none" readonly required>
                ...
                <div>
                    ...
                    <input type="hidden" id="hiddenStatusInput" name="status" value="{{ old('status', 'pending') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date') }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Target Selesai</label>
                    <input type="date" name="target_date" value="{{ old('target_date') }}" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Upload Foto / Lampiran Lapangan</label>
        ...
        <div class="bg-white p-6 rounded-xl shadow mb-6 border border-gray-100">
            <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Tambahkan tahap pengerjaan beserta poin-poin kegiatan dan deskripsinya secara fleksibel.</p>
                </div>
            ...
            {{-- Container Tempat Tahapan Bertambah --}}
            <div id="stagesContainer" class="space-y-6">
                {{-- Tahap awal ditambahkan via JavaScript --}}
            </div>
        </div>
...
<script>
    ...
    // FUNGSI UTAMA 2: Re-indexing seluruh tahap yang tampil di layar
            ...
            <div class="stage-card border border-gray-200 rounded-xl p-5 bg-slate-50 relative transition hover:border-blue-300" id="stage_${stageCounter}">
                <div class="flex items-center justify-between gap-4 mb-4 pb-3 border-b border-gray-200">
                    <div class="flex items-center gap-2 flex-grow">
                        ...
                        <input type="text" name="stages[${stageCounter}][title]" class="stage-title-input w-full border border-gray-300 rounded-lg p-2 text-sm font-semibold focus:ring-2 focus:ring-blue-500 bg-white" placeholder="Contoh: Tahap 1 - Persiapan Awal" required>
                    </div>
        ...
        const pointHtml = `
            <div class="point-item bg-white p-3 rounded-lg border border-gray-200 space-y-2 relative shadow-sm" id="point_${sIndex}_${pIndex}">
                <div class="flex items-center gap-2">
                    ...
                    <input type="text" name="stages[${sIndex}][items][${pIndex}][title]" class="w-full border border-gray-300 rounded-md p-2 text-xs font-semibold focus:ring-2 focus:ring-blue-500" placeholder="Judul Poin (Contoh: 1.1 Identifikasi Proyek)" required>

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Kelola Akun Klien') }}
        </h2>
    </x-slot>

    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <h2 class="text-sm font-bold text-gray-500 uppercase tracking-wider">Daftar Akun Klien</h2>
            <a href="{{ route('admin.clients.create') }}" class="inline-flex items-center justify-center bg-[#0e1d82] hover:bg-[#0c196e] text-white font-medium text-sm py-2 px-4 rounded-lg transition shadow-sm">
                Tambah Klien Baru
            </a>
        </div>
            ...
            <div class="p-6 text-gray-900 overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($clients as $index => $client)
                            <tr class="bg-white hover:bg-slate-50/80 transition">
                                ...
                                <td class="px-6 py-4 space-x-4 text-center">
                                    <a href="{{ route('admin.clients.edit', $client->id) }}" class="font-semibold text-amber-600 hover:text-amber-700 hover:underline">Edit</a>
                                    
                                    <form action="{{ route('admin.clients.destroy', $client->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-400">Belum ada akun klien.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

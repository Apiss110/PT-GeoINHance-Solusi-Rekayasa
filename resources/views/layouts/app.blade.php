...
<html lang="id" class="scroll-smooth overflow-x-hidden">
    <head>
        ...
        {{-- Alpine.js untuk toggle interaktif Hamburger Menu Mobile --}}
        <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>

        {{-- Memanggil CSS & JS via Vite --}}
    ...
    <body class="bg-slate-50 font-sans antialiased text-slate-900 overflow-x-hidden w-full relative">
        ...
        <div class="flex h-screen overflow-hidden">
            ...
            {{-- 🟢 OVERLAY BACKGROUND MOBILE (Muncul saat sidebar terbuka di HP) --}}
            <div 
                x-show="sidebarOpen" 
            ...
            <aside 
                ...
                class="fixed inset-y-0 left-0 z-30 w-64 bg-[#0e1d82] text-white flex flex-col justify-between shrink-0 border-r border-[#0c196e] shadow-lg transform transition-transform duration-300 ease-in-out md:static md:translate-x-0"
            >
                <div class="p-5 flex flex-col h-full overflow-y-auto">
                    
                    {{-- Logo & Tombol Close untuk Mobile --}}
                    <div class="flex items-center justify-between mb-4">
                        <img src="{{ asset('images/logo_inh.png') }}" alt="GeoINHance Logo" class="h-20 w-auto object-contain">
                        
                        <!-- Tombol Close (X) khusus layar HP -->
                        <button @click="sidebarOpen = false" class="md:hidden text-white/70 hover:text-white p-1 rounded-lg focus:outline-none">
                    ...
                    {{-- Tombol Cepat Menuju Halaman Welcome User --}}
                    <div class="mb-6 px-1">
                    ...
                    <div class="mt-auto border-t border-white/10 pt-4 space-y-3">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="w-full">
                            ...
                            <button type="submit" class="w-full flex items-center px-3 py-2 text-xs font-medium text-red-300 hover:bg-red-800/30 rounded-lg transition-all text-left">
                                <svg class="w-4 h-4 mr-2.5 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                {{-- 🟢 TOP BAR KHUSUS MOBILE (Hanya tampil di HP) --}}
                <div class="md:hidden bg-[#0e1d82] text-white flex items-center justify-between px-4 py-3 shrink-0 shadow-md">
                    <div class="flex items-center space-x-3">
                        <!-- Tombol Hamburger Menu Mobile -->
                        <button @click="sidebarOpen = true" class="p-1 rounded-lg text-white hover:bg-white/10 focus:outline-none">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                            </svg>
                        </button>
                        <span class="font-bold text-sm tracking-wide">GeoINHance Admin</span>
                    </div>
                    <img src="{{ asset('images/logo_inh.png') }}" alt="Logo" class="h-8 w-auto object-contain">
                </div>

                @if (isset($header))
                    <header class="bg-white border-b border-slate-200 py-4 px-4 md:py-5 md:px-8 shrink-0">
                        <div class="max-w-7xl">
                            {{ $header }}
                        </div>
                    </header>
                ...
                {{-- Konten Utama (Padding disesuaikan untuk HP) --}}

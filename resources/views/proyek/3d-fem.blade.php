    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div id="projectGridFem" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @foreach($projects as $project)
                <div class="project-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col justify-between hover:shadow-md transition group"
                     ...
                     data-kategori="{{ strtolower(trim($project->category_slug ?? '3d-fem')) }}"
                    ...
                    <div>
                        <div class="bg-slate-900 h-44 flex items-center justify-center relative overflow-hidden">
                            ...
                            @else
                                ...
                                <i class="fa-solid {{ Str::contains(strtolower($project->title), ['dinamis', 'burst', 'eksplisit', 'impact']) || $project->category_slug == 'dinamis-eksplisit' ? 'fa-burst' : 'fa-gears' }} text-5xl text-blue-500/20 group-hover:scale-110 transition duration-300"></i>
                            @endif
                        </div>
                            ...
                            <div class="space-y-2 text-xs border-t border-b border-gray-100 py-3 my-2 text-slate-600">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-[11px]">
                                        {{ $project->category_name ?? (str_contains($project->category_slug, 'dinamis') ? __('filter.category.dynamic') : __('filter.category.static')) }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center">
        ...
        @if($projects->isEmpty())
            <div class="text-center py-12 text-slate-500">
                Belum ada proyek untuk kategori 3D FEM Analysis saat ini.
            </div>
        @endif
        ...
        <div class="mt-16 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-0 sm:justify-between border-t border-gray-200 pt-6">
            <div class="text-sm text-slate-500 font-medium">
                {{ __('portfolio.showing') }} <span id="countDisplayedFem" class="text-slate-700 font-bold">{{ $projects->count() }}</span> {{ __('portfolio.of') }} <span id="countTotalFem" class="text-slate-700 font-bold">{{ $projects->count() }}</span> {{ __('portfolio.results') }}
            </div>

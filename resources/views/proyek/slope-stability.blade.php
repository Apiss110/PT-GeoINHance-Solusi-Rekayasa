...
<section class="bg-slate-950 text-white py-20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <nav class="flex mb-4 text-sm text-slate-400 font-medium">
            ...
            <span class="text-blue-400">{{ $category->name ?? __('slope.title') }}</span>
        </nav>
        
        <h1 class="text-3xl md:text-4xl font-bold tracking-tight">{{ $category->name ?? __('slope.title') }}</h1>
        
        {{-- UPDATE: Menggunakan {!! !!} agar tag HTML di dalam translasi/deskripsi bisa ter-render --}}
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form id="filterFormSlope" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="selectKategoriSlope" class="w-full bg-slate-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition cursor-pointer">
                        ...
                        <option value="{{ $category->slug ?? 'slope-stability-analysis' }}">{{ $category->name ?? 'Slope Stability Analysis' }}</option>
                    </select>
                </div>
        ...
        <div class="mb-8 flex justify-between items-center border-b border-gray-200 pb-4">
            <span class="text-sm text-slate-600 font-medium">
                {{ __('portfolio.list_title') }} <strong class="text-slate-900">{{ $category->name ?? 'Slope Stability Analysis' }}</strong>
            </span>
        ...
        <div id="projectGridSlope" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @foreach($projects as $project)
                <div class="project-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col justify-between hover:shadow-md transition group"
                     ...
                     data-kategori="{{ $category->slug ?? 'slope-stability-analysis' }}"
                    ...
                    <div>
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="space-y-2 text-xs border-t border-b border-gray-100 py-3 my-2 text-slate-600">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-[11px]">
                                        {{ $category->name ?? 'Slope Stability Analysis' }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center">
                            ...
                            {{-- JIKA deskripsi di dalam card project ($project->description) juga punya tag HTML, ubah {{ }} menjadi {!! !!} --}}
        ...
        <div class="mt-16 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-0 sm:justify-between border-t border-gray-200 pt-6">
            <div class="text-sm text-slate-500 font-medium">
                {{ __('portfolio.showing') }} <span id="countDisplayedSlope" class="text-slate-700 font-bold">{{ $projects->count() }}</span> {{ __('portfolio.of') }} <span id="countTotalSlope" class="text-slate-700 font-bold">{{ $projects->count() }}</span> {{ __('portfolio.results') }}
            </div>

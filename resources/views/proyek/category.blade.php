...
@php
    ...
    // Resolusi bahasa untuk Kategori
    $categoryName = $isEn && !empty($category->name_en) ? $category->name_en : ($category->name ?? 'Detail Kategori');
    $categoryDesc = $isEn && !empty($category->description_en) ? $category->description_en : ($category->description ?? $category->content ?? '');
@endphp
    ...
    <div class="container mx-auto px-6 max-w-7xl">
            ...
            <div class="flex justify-end items-center">
                ...
                <div class="relative w-full sm:w-80">
                    ...
                    <input
                        type="text"
                        id="searchInput"
                        placeholder="{{ __('portfolio.search_placeholder') ?? ($isEn ? 'Search projects in this sector...' : 'Cari Proyek di Sektor Ini...') }}"
        ...
        <div class="mb-8 flex flex-wrap justify-between items-center border-b border-slate-200 pb-4 max-w-7xl mx-auto gap-2">
            <span class="text-sm text-slate-600 font-medium">
                {{ __('portfolio.list_title') ?? ($isEn ? 'Project List:' : 'Daftar Proyek:') }} <strong class="text-slate-900">{{ $categoryName }}</strong>
            </span>
            ...
            @if(request('from') == 'all')
                <a href="{{ route('proyek.semua') }}" class="text-xs font-bold text-[#c80000] hover:text-slate-900 transition flex items-center gap-1 uppercase tracking-wider">
                    <i class="fa-solid fa-arrow-left"></i> {{ __('portfolio.back_to_all') ?? ($isEn ? 'Back to All Projects' : 'Kembali ke Semua Proyek') }}
                </a>
            @endif
        </div>
        ...
        <div class="max-w-7xl mx-auto">
            <div id="projectGridDynamic" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                @foreach($projects as $project)
                    @php
                        // Resolusi bahasa per item proyek
                        $projectTitle = $isEn && !empty($project->title_en) ? $project->title_en : $project->title;
                        $projectDesc = $isEn && !empty($project->description_en) ? $project->description_en : $project->description;
                        $projectLoc = $isEn && !empty($project->location_en) ? $project->location_en : $project->location;
                    @endphp
                    ...
                    <div class="project-card bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-md hover:-translate-y-1 transition duration-300"
                         data-nama="{{ strtolower(trim($projectTitle)) }}"
                         data-tahun="{{ $project->year }}"
                         data-kategori="{{ $category->slug }}"
                         data-lokasi="{{ strtolower(trim($projectLoc)) }}">
                        
                        {{-- Foto Cover Proyek --}}
                        <div class="relative h-56 overflow-hidden bg-slate-900">
                            @if($project->image_path)
                                <img src="{{ asset('storage/' . $project->image_path) }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" alt="{{ $projectTitle }}">
                            @else
                        ...
                        <div class="p-6 flex-1 flex flex-col justify-between">
                            ...
                            <a href="{{ route('proyek.detail', $project->id) }}" class="inline-flex items-center text-xs font-bold text-[#c80000] hover:text-slate-900 uppercase tracking-wider transition mt-auto">
                                {{ __('portfolio.read_more') ?? ($isEn ? 'Read More' : 'Selengkapnya') }}
                ...
                <div id="noProjectMessageDynamic" class="hidden col-span-full text-center py-16 bg-white rounded-3xl border border-dashed border-slate-200 p-8 max-w-5xl mx-auto">
                    ...
                    <h4 class="text-sm font-bold text-slate-800 mb-1">
                        {{ $isEn ? 'No Matches Found' : 'Tidak Ada Kecocokan' }}
                    </h4>
                    <p class="text-xs text-slate-500 max-w-sm mx-auto">
                        {{ $isEn ? 'No projects match your search query.' : 'Belum ada proyek yang cocok dengan kata kunci pencarian Anda.' }}
                    </p>
                </div>
            ...
            @if($projects->isEmpty())
                <div class="text-center py-16 bg-white rounded-3xl border border-dashed border-slate-200 p-8 max-w-5xl mx-auto mt-4">
                    ...
                    <h4 class="text-sm font-bold text-slate-800 mb-1">
                        {{ $isEn ? 'No Projects Available' : 'Belum Ada Proyek' }}
                    </h4>
                    <p class="text-xs text-slate-500 max-w-sm mx-auto">
                        {{ $isEn ? 'There are currently no strategic projects registered under this category.' : 'Saat ini belum ada data proyek strategis yang didaftarkan untuk kategori ini.' }}
                    </p>
                </div>
            @endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function applyFiltersAndPagination() {
        ...
        // 4. Update Teks Tampilan Informasi Info Records
        if (totalItems === 0) {
            if (infoStart) infoStart.textContent = 0;
            if (infoEnd) infoEnd.textContent = 0;
            if (infoTotal) infoTotal.textContent = 0;
            noMessage.classList.remove('hidden');
            if (paginationSection) paginationSection.classList.add('hidden');
        } else {
            if (infoStart) infoStart.textContent = startIndex + 1;

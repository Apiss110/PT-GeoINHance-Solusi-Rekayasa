...
<section class="bg-slate-950 text-white py-20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <nav class="flex mb-4 text-sm text-slate-400 font-medium">
            ...
            <span class="text-blue-400">{{ __('review.title') }}</span>
        </nav>
        
        <h1 class="text-3xl md:text-4xl font-bold tracking-tight">{{ __('review.title') }}</h1>
...
<section class="py-12 bg-slate-50 min-h-[50vh]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200/80 mb-10">
            <form id="filterFormReview" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="inputNamaReview" placeholder="{{ __('filter.search_placeholder_review') }}" 
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="selectTahunReview" class="w-full bg-slate-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition cursor-pointer">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="selectKategoriReview" class="w-full bg-slate-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition cursor-pointer">
                        <option value="">{{ __('filter.category_all') }}</option>
                        <option value="audit-struktur">{{ __('filter.category.audit_struktur') }}</option>
                        <option value="peer-review">{{ __('filter.category.peer_review') }}</option>
                    </select>
                </div>

                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('filter.location_label') }}</label>
                    <select id="selectLokasiReview" class="w-full bg-slate-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition cursor-pointer">
                        <option value="">{{ __('filter.location_all') }}</option>
                        <option value="jakarta">{{ __('filter.location.jakarta') }}</option>
                        <option value="kalimantan">{{ __('filter.location.kalimantan') }}</option>
                    </select>
                </div>
        ...
        <div class="mb-8 flex justify-between items-center border-b border-gray-200 pb-4">
            <span class="text-sm text-slate-600 font-medium">
                {{ __('portfolio.list_title') }} <strong class="text-slate-900">{{ __('review.title') }}</strong>
            </span>
        ...
        <div id="projectGridReview" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="project-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col justify-between hover:shadow-md transition group"
                 data-nama="{{ Str::lower(__('project.review.1.title')) }}"
                 data-tahun="2024"
                 data-kategori="audit-struktur"
                 data-lokasi="jakarta">
                <div>
                    <div class="bg-slate-900 h-44 flex items-center justify-center relative overflow-hidden">
                        ...
                        <i class="fa-solid fa-building-shield text-5xl text-blue-500/20 group-hover:scale-110 transition duration-300"></i>
                    </div>
                    
                    <div class="p-6 space-y-4">
                        <h3 class="text-base font-bold text-gray-900 group-hover:text-blue-600 transition min-h-[3rem] line-clamp-2">
                            {{ __('project.review.1.title') }}
                        </h3>
                        ...
                        <div class="space-y-2 text-xs border-t border-b border-gray-100 py-3 my-2 text-slate-600">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-[11px]">{{ __('filter.category.audit_struktur') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-400 font-medium">{{ __('project.label.location') }}</span>
                                <span class="font-semibold text-slate-900">{{ __('project.review.1.location') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                ...
                <div class="p-6 pt-0 space-y-3">
                    <div class="flex flex-wrap gap-1">
                        <span class="bg-slate-50 text-slate-600 text-[10px] font-mono px-2 py-0.5 rounded border border-slate-200">ETABS v21</span>
                    </div>
            ...
            <div class="project-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col justify-between hover:shadow-md transition group"
                 data-nama="{{ Str::lower(__('project.review.2.title')) }}"
                 data-tahun="2023"
                 data-kategori="peer-review"
                 data-lokasi="kalimantan">
                <div>
                    <div class="bg-slate-900 h-44 flex items-center justify-center relative overflow-hidden">
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent opacity-80 z-10"></div>
                        <i class="fa-solid fa-bridge text-5xl text-blue-500/20 group-hover:scale-110 transition duration-300"></i>
                    </div>
                    
                    <div class="p-6 space-y-4">
                        <h3 class="text-base font-bold text-gray-900 group-hover:text-blue-600 transition min-h-[3rem] line-clamp-2">
                            {{ __('project.review.2.title') }}
                        </h3>
                        
                        <div class="space-y-2 text-xs border-t border-b border-gray-100 py-3 my-2 text-slate-600">
                            <div class="flex justify-between items-center">
                                <span class="text-slate-400 font-medium">{{ __('project.label.category') }}</span>
                                <span class="font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded text-[11px]">{{ __('filter.category.peer_review') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-400 font-medium">{{ __('project.label.location') }}</span>
                                <span class="font-semibold text-slate-900">{{ __('project.review.2.location') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-400 font-medium">{{ __('project.label.year') }}</span>
                ...
                <div class="p-6 pt-0 space-y-3">
                    <div class="flex flex-wrap gap-1">
                        <span class="bg-slate-50 text-slate-600 text-[10px] font-mono px-2 py-0.5 rounded border border-slate-200">Midas Civil</span>
                    </div>
                    <a href="#" class="block text-center bg-slate-900 text-white py-2 rounded-lg text-xs font-semibold hover:bg-red-800 transition">
                        {{ __('portfolio.read_more') }}
                    </a>
            ...
            <div id="noProjectMessageReview" class="hidden col-span-full text-center py-12 text-slate-500 font-medium bg-white rounded-xl border border-gray-200">
                <i class="fa-solid fa-folder-open text-3xl text-slate-300 mb-2 block"></i>
                {{ __('portfolio.empty_review') }}
            </div>
        ...
        <div class="mt-16 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-0 sm:justify-between border-t border-gray-200 pt-6">
            <div class="text-sm text-slate-500 font-medium">
                {{ __('portfolio.showing') }} <span id="countDisplayedReview" class="text-slate-700 font-bold">2</span> {{ __('portfolio.of') }} <span id="countTotalReview" class="text-slate-700 font-bold">2</span> {{ __('portfolio.results') }}
            </div>
...
<script>
document.getElementById('filterFormReview').addEventListener('submit', function(e) {
    e.preventDefault(); // Mengunci reload halaman semenit

    // Menangkap pilihan filter milik user
    const filterNama = document.getElementById('inputNamaReview').value.toLowerCase().trim();
    const filterTahun = document.getElementById('selectTahunReview').value;
    const filterKategori = document.getElementById('selectKategoriReview').value;
    const filterLokasi = document.getElementById('selectLokasiReview').value;
    ...
    document.getElementById('countDisplayedReview').innerText = displayedCount;
    
    // Tampilkan pesan jika kosong pencarian
    const noMessage = document.getElementById('noProjectMessageReview');
    if (displayedCount === 0) {
